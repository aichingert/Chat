<script lang="ts">
    ...
    const formatDate = (written_at : number) => {
        ...
        return `${date.toLocaleDateString()} at ${date.getHours()}:${date.getMinutes() > 10 ? date.getMinutes() : `0${date.getMinutes()}`}`
    }
